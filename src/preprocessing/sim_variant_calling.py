...
counts_file = '/homes/buettner/research/users/buettner/hipsci-singlecell/data/pilot3/iPS/counts/counts.hdf5'
...
if __name__ == '__main__':
    if 'cluster' in sys.argv:
        #run merge jobs
        expr_file = counts_file#sys.argv[4]
        ...
        fout.create_dataset(name='error',data=error)
        fout.create_dataset(name='Itop',data=Itop)
    else:
        ...
        Ngenes = len(Mcounts)
        Itop = Mcounts.argsort()[::-1][0:Ngenes]
        ...
        Iok = (SP.array(mapped)>0.5) & (SP.array(count)>100)
            
        errorOK = SP.array(error)[Iok]
        error = SP.array(error)
