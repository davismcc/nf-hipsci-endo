'
Read expression values from kallisto and return .rds files with SCESets

Usage:
  read_kallisto_to_scesets.R [--input_dir <DIRECTORY_IN> --output_prefix <OUT_PREFIX> --biomart <HOST> -rqhv]

Options:

  -i --input_dir <DIRECTORIES_IN>  input directory containing sub-directories with kallisto quantification results
...
This program reads the kallisto results into an SCESet object (from the
...
main <- function(input_dir, output_prefix, host = NULL, verbose = TRUE) {
    ...
    featureNames(sce_tx) <- paste(
        featureNames(sce_tx), fData(sce_tx)$hgnc_symbol, sep = "_")
    ...
    ## tweak feature names to be more readable
    featureNames(sce_gene) <- paste(
        featureNames(sce_gene), fData(sce_gene)$hgnc_symbol, sep = "_")

    ## calculate QC metrics
    ercc_genes <- grepl("^ERCC", featureNames(sce_gene))
    mt_genes <- grepl("^MT-", fData(sce_gene)$hgnc_symbol)
