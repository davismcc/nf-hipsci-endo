...
if __name__=='__main__':
    ...
    ## retrieve and define interaction terms
    I0 = sp.ones((n_cells,1))
    # the null model will contain (I0 * snp) \alpha
    # (in this case a commmon effect across cancer types)
    I = fexprs['interaction'][:]
    I = I[sample_gene_idx,:]
    ...
    Kall = fK['Kpop'][:]
    ## expand to big K for cells
    K = Kall[sample_K_idx,:][:,sample_K_idx]
    K /= K.diagonal().mean()
    ...
    for gene in genes:
        ...
        if fit_design:
            lmm = QTL.test_interaction_lmm(X_gene, Y_gene, K=K, covs = design, Inter=I, Inter0=I0) 
        else:
            lmm = QTL.test_interaction_lmm(X_gene, Y_gene, K=K, Inter=I, Inter0=I0) 
        pv = lmm.getPv()
